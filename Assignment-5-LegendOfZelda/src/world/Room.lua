...
function Room:init(player)
    ...
    Event.on('pot-thrown', function(params)
        ...
        local distance = math.random(4, 5) == 4 and (4 * TILE_SIZE) or (5 * TILE_SIZE)
        local finalPosX, finalPosY = pot.x, pot.y 
        local offset = 0
        if params.direction == 'up' then
            offset = 0
            finalPosY = pot.y - distance + TILE_SIZE
        elseif params.direction == 'down' then
            offset = 0
            finalPosY = pot.y + distance - TILE_SIZE
        elseif params.direction == 'left' then
            finalPosX = pot.x - distance + TILE_SIZE
...
function Room:update(dt)
        ...
        if entity.health <= 0 and not entity.dead then
            if math.random(10) >= 5 then
                ...
                local heart = GameObject(
                    ...
                    self.player.health = math.min(self.player.health,6)
                end
    ...
    for k, object in pairs(self.objects) do
        if object.type == 'pot' and object.state == 'thrown' then
            if object.x > self.player.x + (4 * TILE_SIZE) then
                object.state = 'broken'
            elseif object.x <= self.player.x - (4 * TILE_SIZE) then
                object.state = 'broken'
            elseif object.y >= self.player.y + (4 * TILE_SIZE) then
                object.state = 'broken'
            elseif object.y <= self.player.y - (4 * TILE_SIZE) then
                object.state = 'broken'
            end
        ...
        if love.keyboard.wasPressed('return') then
            if object.type == 'pot' and object.state == 'default' then
                if self.player.potCollider:collides(object) then
