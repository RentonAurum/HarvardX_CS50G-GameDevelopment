...
function Level:init()
    ...
    self.aliensContainer={}
    self.alienCollided = false
    self.alienSplitted = false
    self.emptyAliens = false
...
----------- RESET -----------
for k, aliensContainer in pairs(self.aliensContainer) do

    local xPos,yPos = aliensContainer.body:getPosition()
    local xVel, yVel = aliensContainer.body:getLinearVelocity()

    if xPos < 0 or (math.abs(xVel) + math.abs(yVel) < 1.5) then
        
        aliensContainer.body:destroy()
        table.remove(self.aliensContainer, k)

        -- re-initialize level if we have no more aliens
        if #self.aliensContainer == 0 then
            self.emptyAliens = true
        end
    end

end
    ----------- RESET IF NO BIRDS -----------
    if self.launchMarker.launched and self.emptyAliens then
    ...
    if love.keyboard.isDown ("space") and self.launchMarker.launched and not self.alienCollided and not self.alienSplitted then

        local xPos = self.launchMarker.alien.body:getX()
        local yPos = self.launchMarker.alien.body:getY()
        local xVel, yVel = self.launchMarker.alien.body:getLinearVelocity()
        
        local alienUp = Alien(self.world, 'round', xPos -5, yPos - 30, 'Player')
        local alienDown = Alien(self.world, 'round', xPos -5, yPos + 30, 'Player')

        alienUp.body:setLinearVelocity(xVel + 10, yVel - 10)
        alienUp.fixture:setRestitution(0.2)
        alienUp.body:setAngularDamping(1)
        alienUp.sprite = 10
        table.insert(self.aliensContainer, alienUp)

        alienDown.body:setLinearVelocity(xVel + 10, yVel + 10)
        alienDown.fixture:setRestitution(0.2)
        alienDown.body:setAngularDamping(1)
        alienDown.sprite = 15
        table.insert(self.aliensContainer, alienDown)
...
function Level:render()
    ...
    for k, player in pairs(self.aliensContainer) do
        player:render()
    end
