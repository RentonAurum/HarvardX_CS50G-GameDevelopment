...
function PlayState:update(dt)
    ...
    if self.canInput then
        ...
        local mX = mX - (VIRTUAL_WIDTH - 272)
        local mY = mY - 16

        local posX = math.floor(mX / 32)
        local posY = math.floor (mY / 32)

            if ((self.lastMouseposX ~= posX or self.lastMouseposY ~= posY) and (posX >= 0 and posX <= 8) and (posY >= 0 and posY <=8)) then
                if ((self.boardHighlightX ~= posX or self.boardHighlightY ~= posY)) then
                    gSounds['select']:play()
                end
                self.boardHighlightX = posX
                self.boardHighlightY = posY
            end
            ...
            elseif math.abs(self.highlightedTile.gridX - x) + math.abs(self.highlightedTile.gridY - y) > 1 then
                ...
                :finish(function()
                    ...
                    if matches then
                        ...
                        self.board:switch(self.highlightedTile, newTile)
                        Timer.tween(0.1, {[self.highlightedTile] = {x = newTile.x, y = newTile.y},
                            [newTile] = {x = self.highlightedTile.x, y = self.highlightedTile.y}
                        })
...
function PlayState:switch(t1, t2, ckeckFail)
    self.board:switch(t1, t2)
end
...
function PlayState:calculateMatches()
    ...
    if matches then
        ...
        if not self.board:checkOut() then
            while not self.board:checkOut() do
