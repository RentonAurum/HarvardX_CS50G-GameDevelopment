...
function Board:checkOut()
    for y = 1, 8 do
        for x = 1, 8 do
            --
            if x > 1 then
                self:switch(self.tiles[y][x], self.tiles[y][x - 1])
...
--f
            ...
            if y > 1 then
                self:switch(self.tiles[y][x], self.tiles[y - 1][x])
                if self:calculateMatches() then
                    self:switch(self.tiles[y][x], self.tiles[y - 1][x])
                    return true
                end
